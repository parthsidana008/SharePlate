...
const EditDonation = ({ donation, onCancel, onSubmit }) => {
  const [title, setTitle] = useState(donation?.title || '');
  const [description, setDescription] = useState(donation?.description || '');
  const [type, setType] = useState(donation?.type || 'Veg');
  const [quantity, setQuantity] = useState(donation?.quantity || '');
  const [location, setLocation] = useState(donation?.location?.area || donation?.location?.city || donation?.location || '');
  const [expiresIn, setExpiresIn] = useState(donation?.expiresIn || '');
  const [uploadedImage, setUploadedImage] = useState(donation?.imageUrl || null);
  ...
  useEffect(() => {
    if (donation) {
      ...
      setType(donation.type || 'Veg');
      setQuantity(donation.quantity || '');
      setLocation(donation.location?.area || donation.location?.city || donation.location || '');
      setExpiresIn(donation.expiresIn || '');
      setUploadedImage(donation.imageUrl || null);
    }
  }, [donation]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const success = await onSubmit(donation._id || donation.id, {
      title,
      description,
      type,
      quantity,
      ...
      expiresIn,
      imageUrl: uploadedImage || donation.imageUrl
    });
    setLoading(false);
    if (success) {
      onCancel();
    ...
    <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full h-full flex flex-col md:flex-row overflow-hidden">
      ...
      <div className="md:w-1/3 bg-slate-900 text-white p-8 flex flex-col justify-between relative overflow-hidden flex-shrink-0">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-3xl font-bold mb-4 leading-tight">Update Your <span className="text-green-400">Donation</span></h2>
          <p className="text-slate-400 text-sm">Modify your donation details and re-upload image if needed.</p>
        </div>
      ...
      <div className="md:w-2/3 p-8 bg-white overflow-y-auto custom-scrollbar">
          <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-slate-800">Edit Donation Details</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
                type="submit"
                disabled={loading}
                className="px-8 py-2.5 bg-slate-900 text-white rounded-xl shadow-lg shadow-slate-900/20 text-sm font-semibold hover:bg-slate-800 hover:-translate-y-0.5 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
            >
